...
describe('estimateMaintenance', () => {
  ...
  it('prices a bulk at the lower gain density (3100), not fat density (3500)', () => {
    const { entries, nutrition } = scenario(28, { start: 175, lbsPerWeek: 0.5, kcal: 3000 })
    // Bulk started well before the window, so it does not clamp the 27-day span.
    const est = estimateMaintenance(entries, nutrition, [{ start: '2026-06-01', name: 'Bulk' }], TODAY)

    expect(est.kind).toBe('ok')
    // 3000 − (0.5/7·27)·3100/27 ≈ 2779 → 2780. Flat 3500 would give 2750.
    expect(est.maintenance).toBe(2780)
  })

  it('picks the density from the logged phase, not the sign of the scale trend', () => {
    // Bulk phase, but the modelled weight is drifting *down* over the window (water settling).
    const { entries, nutrition } = scenario(28, { start: 175, lbsPerWeek: -0.5, kcal: 3000 })
    const est = estimateMaintenance(entries, nutrition, [{ start: '2026-06-01', name: 'Bulk' }], TODAY)

    expect(est.kind).toBe('ok')
    // Gain density despite the down trend: 3000 + (0.5/7·27)·3100/27 ≈ 3221 → 3220.
    // Fat density (what Math.sign alone would pick for a loss) would give 3250.
    expect(est.maintenance).toBe(3220)
  })
})

describe('targetIntake / intakeAdjustment', () => {
  it('shifts maintenance by the weekly goal in daily kcal, at loss density for a cut', () => {
    expect(targetIntake(2500, -1)).toBe(2000)
    expect(targetIntake(2500, 0)).toBe(2500)
  })

  it('prices a gain goal at the lower gain density, not fat density', () => {
    // +0.5 lb/wk at 3100 kcal/lb: +1550/7 ≈ +221 → 3220, vs the old flat-3500 3250.
    expect(targetIntake(3000, 0.5)).toBe(3220)
